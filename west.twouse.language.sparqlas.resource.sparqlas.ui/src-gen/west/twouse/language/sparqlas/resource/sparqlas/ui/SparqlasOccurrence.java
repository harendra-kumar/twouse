...
public class SparqlasOccurrence {
	...
	private west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasTokenScanner tokenScanner;
	 ...
	 */
	public SparqlasOccurrence(west.twouse.language.sparqlas.resource.sparqlas.ISparqlasTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new west.twouse.language.sparqlas.resource.sparqlas.mopp.SparqlasMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
