 ...
 */
public class YumlMinimalModelHelper {
	
	private final static west.twouse.language.yuml.resource.yuml.util.YumlEClassUtil eClassUtil = new west.twouse.language.yuml.resource.yuml.util.YumlEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + west.twouse.language.yuml.resource.yuml.util.YumlStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = west.twouse.language.yuml.resource.yuml.util.YumlListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if(attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + west.twouse.language.yuml.resource.yuml.util.YumlStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = west.twouse.language.yuml.resource.yuml.util.YumlListUtil.castListUnchecked(value);
