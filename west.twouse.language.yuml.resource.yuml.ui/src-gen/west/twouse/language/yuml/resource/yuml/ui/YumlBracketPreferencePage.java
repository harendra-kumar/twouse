 ...
 * </ul>
 */
public class YumlBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new west.twouse.language.yuml.resource.yuml.mopp.YumlMetaInformation().getSyntaxName();
	
	private west.twouse.language.yuml.resource.yuml.ui.YumlBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public YumlBracketPreferencePage() {
		super();
		
		west.twouse.language.yuml.resource.yuml.IYumlMetaInformation metaInformation = new west.twouse.language.yuml.resource.yuml.mopp.YumlMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(west.twouse.language.yuml.resource.yuml.ui.YumlUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new west.twouse.language.yuml.resource.yuml.ui.YumlBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					west.twouse.language.yuml.resource.yuml.IYumlBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					west.twouse.language.yuml.resource.yuml.IYumlBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(		west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(		language + west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof west.twouse.language.yuml.resource.yuml.ui.YumlEditor) {
			((west.twouse.language.yuml.resource.yuml.ui.YumlEditor) editor).invalidateTextRepresentation();
		}
	}
