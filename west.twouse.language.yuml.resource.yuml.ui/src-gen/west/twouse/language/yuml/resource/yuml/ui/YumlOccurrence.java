 ...
 */
public class YumlOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "west.twouse.language.yuml.resource.yuml.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "west.twouse.language.yuml.resource.yuml.ui.occurences.declaration";
	...
	private west.twouse.language.yuml.resource.yuml.ui.YumlTokenScanner tokenScanner;
	...
	private west.twouse.language.yuml.resource.yuml.IYumlTextResource textResource;
	 ...
	 */
	public YumlOccurrence(west.twouse.language.yuml.resource.yuml.IYumlTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, west.twouse.language.yuml.resource.yuml.ui.YumlTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new west.twouse.language.yuml.resource.yuml.mopp.YumlMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(west.twouse.language.yuml.resource.yuml.ui.YumlBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, west.twouse.language.yuml.resource.yuml.ui.YumlPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, west.twouse.language.yuml.resource.yuml.ui.YumlPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, west.twouse.language.yuml.resource.yuml.ui.YumlPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == west.twouse.language.yuml.resource.yuml.ui.YumlPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(west.twouse.language.yuml.resource.yuml.ui.YumlOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(west.twouse.language.yuml.resource.yuml.ui.YumlOccurrence.DECLARATION_ANNOTATION_ID);
	}
