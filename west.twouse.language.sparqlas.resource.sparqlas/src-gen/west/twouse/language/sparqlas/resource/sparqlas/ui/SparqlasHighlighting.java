...
public class SparqlasHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private final static west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionHelper positionHelper = new west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionHelper();
	private boolean isHighlightBrackets = true;
	private boolean isHighlightOccurrences = true;
	private west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasTokenScanner scanner;
	...
	private west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasOccurrence occurrence;
	private west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasBracketSet bracketSet;
	...
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, west.twouse.language.sparqlas.resource.sparqlas.ISparqlasBackgroundParsingListener {
	...
	public SparqlasHighlighting(west.twouse.language.sparqlas.resource.sparqlas.ISparqlasTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasColorManager colorManager, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = west.twouse.language.sparqlas.resource.sparqlas.mopp.SparqlasPlugin.getDefault().getPreferenceStore();
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasTokenScanner(colorManager);
		occurrence = new west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasBracketSet(sourceviewer, "sparqlas");
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		isHighlightOccurrences = preferenceStore.getBoolean(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPreferenceConstants.EDITOR_OCCURRENCE_CHECKBOX);
		definitionColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPreferenceConstants.EDITOR_DEFINITION_COLOR));
		proxyColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPreferenceConstants.EDITOR_PROXY_COLOR));
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasEditor editor) {
	...
	private void setHighlighting() {
		...
		if (occurrence.isPositionsChanged()) {
			...
			west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.DEFINTION.toString());
			setCategoryHighlighting(document,
			west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.PROXY.toString());
		}
		setCategoryHighlighting(document, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.BRACKET.toString());
	}
	...
	private void setCategoryHighlighting(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.PROXY.toString())) {
		...
		for (org.eclipse.jface.text.Position position : positions) {
			...
			if (tmpPosition != null) {
				if (category.equals(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.DEFINTION.toString())) {
				...
				if (category.equals(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.BRACKET.toString())) {
					styleRange = getStyleRangeAtPosition(tmpPosition);
	...
	private void removeHighlighting() {
		org.eclipse.jface.text.IDocument document = projectionViewer.getDocument();
		removeHighlightingCategory(document, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.BRACKET.toString());
		if (occurrence.isToRemoveHighlighting()) {
			removeHighlightingCategory(document, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.DEFINTION.toString());
			removeHighlightingCategory(document, west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.PROXY.toString());
			lastStyleRange = null;
		}
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, category);
		boolean isOccurrence = (category.equals(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.DEFINTION.toString()) || category.equals(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.PROXY.toString())) && lastStyleRange != null;
		if (category.equals(west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new west.twouse.language.sparqlas.resource.sparqlas.ui.SparqlasEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
	...
	private void handleContentOutlineSelection(org.eclipse.jface.viewers.ISelection selection) {
		if (!selection.isEmpty()) {
			...
			if (selectedElement instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (resource instanceof west.twouse.language.sparqlas.resource.sparqlas.ISparqlasTextResource) {
