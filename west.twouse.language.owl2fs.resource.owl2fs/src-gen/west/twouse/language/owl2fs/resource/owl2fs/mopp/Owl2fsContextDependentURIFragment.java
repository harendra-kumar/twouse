...
public abstract class Owl2fsContextDependentURIFragment<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> implements west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsContextDependentURIFragment<ReferenceType> {
	...
	protected west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsReferenceResolveResult<ReferenceType> result;
	...
	public synchronized west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsReferenceResolveResult<ReferenceType> resolve() {
		if (resolving) {
			return null;
		}
		resolving = true;
	...
	public abstract west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsReferenceResolver<ContainerType, ReferenceType> getResolver();
	...
	private void handleMultipleResults() {
		...
		if (temp instanceof org.eclipse.emf.common.util.EList<?>) {
			list = west.twouse.language.owl2fs.resource.owl2fs.util.Owl2fsCastUtil.cast(temp);
		}
		
		boolean first = true;
		for(west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsReferenceMapping<ReferenceType> mapping : result.getMappings()) {
			if (first) {
				first = false;
			} else if (list != null) {
				addResultToList(mapping, proxy, list);
			} else {
				west.twouse.language.owl2fs.resource.owl2fs.mopp.Owl2fsPlugin.logError(container.eClass().getName() + "." + reference.getName() + " has multiplicity 1 but was resolved to multiple elements", null);
			}
		}
	}
	
	private void addResultToList(west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsReferenceMapping<ReferenceType> mapping, org.eclipse.emf.ecore.EObject proxy, org.eclipse.emf.common.util.EList<org.eclipse.emf.ecore.EObject> list) {
		org.eclipse.emf.ecore.EObject target = null;
		int proxyPosition = list.indexOf(proxy);
		
		if (mapping instanceof west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsElementMapping<?>) {
			target = ((west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsElementMapping<ReferenceType>) mapping).getTargetElement();
		} else if (mapping instanceof west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsURIMapping<?>) {
			target = org.eclipse.emf.ecore.util.EcoreUtil.copy(proxy);
			org.eclipse.emf.common.util.URI uri = ((west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsURIMapping<ReferenceType>) mapping).getTargetIdentifier();
