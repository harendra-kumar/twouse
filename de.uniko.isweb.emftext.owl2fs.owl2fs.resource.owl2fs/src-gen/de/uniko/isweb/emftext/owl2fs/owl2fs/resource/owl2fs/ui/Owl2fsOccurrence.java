...
public class Owl2fsOccurrence {
	...
	private de.uniko.isweb.emftext.owl2fs.owl2fs.resource.owl2fs.ui.Owl2fsTokenScanner tokenScanner;
	...
	private de.uniko.isweb.emftext.owl2fs.owl2fs.resource.owl2fs.IOwl2fsTextResource textResource;
	...
	//
	public Owl2fsOccurrence(de.uniko.isweb.emftext.owl2fs.owl2fs.resource.owl2fs.IOwl2fsTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, de.uniko.isweb.emftext.owl2fs.owl2fs.resource.owl2fs.ui.Owl2fsTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new de.uniko.isweb.emftext.owl2fs.owl2fs.resource.owl2fs.mopp.Owl2fsMetaInformation().getTokenNames();
	...
	public void handleOccurrenceHighlighting(de.uniko.isweb.emftext.owl2fs.owl2fs.resource.owl2fs.ui.Owl2fsBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, de.uniko.isweb.emftext.owl2fs.owl2fs.resource.owl2fs.ui.Owl2fsPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, de.uniko.isweb.emftext.owl2fs.owl2fs.resource.owl2fs.ui.Owl2fsPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
