...
public class Owl2fsOccurrence {
	...
	private west.twouse.language.owl2fs.resource.owl2fs.ui.Owl2fsTokenScanner tokenScanner;
	...
	private west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsTextResource textResource;
	 ...
	 */
	public Owl2fsOccurrence(west.twouse.language.owl2fs.resource.owl2fs.IOwl2fsTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, west.twouse.language.owl2fs.resource.owl2fs.ui.Owl2fsTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new west.twouse.language.owl2fs.resource.owl2fs.mopp.Owl2fsMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(west.twouse.language.owl2fs.resource.owl2fs.ui.Owl2fsBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, west.twouse.language.owl2fs.resource.owl2fs.ui.Owl2fsPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, west.twouse.language.owl2fs.resource.owl2fs.ui.Owl2fsPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
