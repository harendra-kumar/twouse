...
/**
 * A text editor for 'yuml' models.
 */
public class YumlEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider {
	
	private west.twouse.language.yuml.resource.yuml.ui.YumlHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private west.twouse.language.yuml.resource.yuml.ui.YumlCodeFoldingManager codeFoldingManager;
	private west.twouse.language.yuml.resource.yuml.ui.YumlBackgroundParsingStrategy bgParsingStrategy = new west.twouse.language.yuml.resource.yuml.ui.YumlBackgroundParsingStrategy();
	private java.util.Collection<west.twouse.language.yuml.resource.yuml.IYumlBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<west.twouse.language.yuml.resource.yuml.IYumlBackgroundParsingListener>();
	private west.twouse.language.yuml.resource.yuml.ui.YumlColorManager colorManager = new west.twouse.language.yuml.resource.yuml.ui.YumlColorManager();
	private west.twouse.language.yuml.resource.yuml.ui.YumlOutlinePage outlinePage;
	...
	private west.twouse.language.yuml.resource.yuml.ui.YumlPropertySheetPage propertySheetPage;
	...
	private west.twouse.language.yuml.resource.yuml.ui.IYumlBracketHandler bracketHandler;
	
	public YumlEditor() {
		super();
		setSourceViewerConfiguration(new west.twouse.language.yuml.resource.yuml.ui.YumlEditorConfiguration(this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), YumlEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								west.twouse.language.yuml.resource.yuml.IYumlTextResource currentResource = getResource();
				...
				west.twouse.language.yuml.resource.yuml.ui.YumlUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("west.twouse.language.yuml.resource.yuml.EditorContext");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new west.twouse.language.yuml.resource.yuml.ui.YumlHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new west.twouse.language.yuml.resource.yuml.ui.YumlCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		west.twouse.language.yuml.resource.yuml.mopp.YumlNature.activate(inputFile.getProject());
		...
		west.twouse.language.yuml.resource.yuml.IYumlTextResource loadedResource = (west.twouse.language.yuml.resource.yuml.IYumlTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				west.twouse.language.yuml.resource.yuml.IYumlTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof west.twouse.language.yuml.resource.yuml.IYumlTextResource) {
					setResource((west.twouse.language.yuml.resource.yuml.IYumlTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					west.twouse.language.yuml.resource.yuml.ui.YumlUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the YumlEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				west.twouse.language.yuml.resource.yuml.ui.YumlUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(west.twouse.language.yuml.resource.yuml.IYumlTextResource resource) {
		assert resource != null;
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new west.twouse.language.yuml.resource.yuml.ui.YumlOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new west.twouse.language.yuml.resource.yuml.ui.YumlPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			west.twouse.language.yuml.resource.yuml.IYumlTextResource textResource = (west.twouse.language.yuml.resource.yuml.IYumlTextResource) element.eResource();
			west.twouse.language.yuml.resource.yuml.IYumlLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			west.twouse.language.yuml.resource.yuml.IYumlTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				west.twouse.language.yuml.resource.yuml.IYumlTextToken token = lexer.getNextToken();
			...
			west.twouse.language.yuml.resource.yuml.ui.YumlUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	public void addBackgroundParsingListener(west.twouse.language.yuml.resource.yuml.IYumlBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (west.twouse.language.yuml.resource.yuml.IYumlBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public west.twouse.language.yuml.resource.yuml.ui.IYumlBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(west.twouse.language.yuml.resource.yuml.ui.IYumlBracketHandler bracketHandler) {
