 ...
 */
public class YumlHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private final static west.twouse.language.yuml.resource.yuml.ui.YumlPositionHelper positionHelper = new west.twouse.language.yuml.resource.yuml.ui.YumlPositionHelper();
	private boolean isHighlightBrackets = true;
	private west.twouse.language.yuml.resource.yuml.ui.YumlTokenScanner scanner;
	...
	private west.twouse.language.yuml.resource.yuml.ui.YumlOccurrence occurrence;
	private west.twouse.language.yuml.resource.yuml.ui.YumlBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, west.twouse.language.yuml.resource.yuml.IYumlBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public YumlHighlighting(west.twouse.language.yuml.resource.yuml.IYumlTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, west.twouse.language.yuml.resource.yuml.ui.YumlColorManager colorManager, west.twouse.language.yuml.resource.yuml.ui.YumlEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = west.twouse.language.yuml.resource.yuml.ui.YumlUIPlugin.getDefault().getPreferenceStore();
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new west.twouse.language.yuml.resource.yuml.ui.YumlTokenScanner(textResource, colorManager);
		occurrence = new west.twouse.language.yuml.resource.yuml.ui.YumlOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new west.twouse.language.yuml.resource.yuml.ui.YumlBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, west.twouse.language.yuml.resource.yuml.ui.YumlPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(west.twouse.language.yuml.resource.yuml.ui.YumlEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, west.twouse.language.yuml.resource.yuml.ui.YumlPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, west.twouse.language.yuml.resource.yuml.ui.YumlPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(west.twouse.language.yuml.resource.yuml.ui.YumlPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new west.twouse.language.yuml.resource.yuml.ui.YumlEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
	...
	private void handleContentOutlineSelection(org.eclipse.jface.viewers.ISelection selection) {
		if (!selection.isEmpty()) {
			...
			if (selectedElement instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (resource instanceof west.twouse.language.yuml.resource.yuml.IYumlTextResource) {
